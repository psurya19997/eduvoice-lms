// Sentence Builder hub — mirrors WFHub, minus the LRU/cooldown logic.
// Level tabs, 10-step grid, tap step to play the frontier session in that step.
...
export default function SBHub() {
  const navigate = useNavigate();
  const { profile, loading: authLoading } = useAuthProfile('student');

  const [loading, setLoading] = useState(true);
  const [progress, setProgress] = useState({
    started: 0, completed: 0, total: 10, frontierStep: 1,
    stepsMeta: [], levelsData: DEFAULT_LEVELS, activeLevelKey: 'alpha',
  });
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const [prog, pts] = await Promise.all([
        loadSbProgress(profile.id, activeTab),
    ...
    return (
      <div className="h-full flex flex-col bg-slate-50 relative overflow-hidden">
        <div className="px-4 pt-3 pb-2 flex items-center justify-between">
          ...
          <div className="text-[15px] font-black text-slate-900">Sentence Builder</div>
          <div className="w-8" />
        </div>
  ...
  const { frontierStep, stepsMeta, completed, total, levelsData } = progress;
  ...
  return (
    <div className="h-full flex flex-col bg-slate-50 relative overflow-y-auto pb-8">
      ...
      <div className="px-4 pt-3 pb-2 flex items-center justify-between gap-2 shrink-0 bg-white border-b border-slate-200">
        <BackButton to="/student/games" />
        <h1 className="text-[16px] font-black text-slate-900 leading-tight">Sentence Builder</h1>
      ...
      <div className="p-4 flex flex-col gap-4 max-w-md mx-auto w-full">
        ...
        {/* Active Level Overview Card */}
            <div className="rounded-3xl bg-gradient-to-br from-indigo-600 to-violet-700 text-white p-5 shadow-lg flex flex-col gap-4 relative overflow-hidden">
              ...
              <button
                type="button"
                onClick={() => navigate(`/student/games/sentence-builder/play?level=${activeTab}&step=${frontierStep}`)}
            ...
            <div className="flex flex-col gap-3 pt-1">
              <div className="flex items-center justify-between px-1">
                <div className="text-[12px] font-black uppercase tracking-wider text-slate-500 capitalize">
                  {currentLevelObj.title || currentLevelObj.key} Steps
                </div>
                ...
                {stepsMeta.map((s) => {
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      disabled={noContent}
                      onClick={() => navigate(`/student/games/sentence-builder/play?level=${activeTab}&step=${s.step}`)}
                      className={`
